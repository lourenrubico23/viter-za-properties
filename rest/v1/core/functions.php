...
function token(
    ...
    if (!empty($token)) {
        try {
            ...
            // last 5 digit of password from database
            $dataLastFive = substr($row["user_password"], -15);

            // last 5 digit of password from login
            $inputLastFive = substr($decoded->data->data->user_password, -15);

            // check value if matched
            $isPasswordMatched = $dataLastFive == $inputLastFive;

            // remove the password from the login object
            ...
            $returnData["data"] = array_merge(
                ...
                array('user_access' => $isPasswordMatched),
            );
            $returnData["count"] = $result->rowCount();
            $returnData["success"] = true;
            $returnData["message"] = "Access granted.";
