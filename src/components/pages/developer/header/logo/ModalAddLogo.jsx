...
const ModalAddLogo = ({ itemEdit, logoData }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        logoData?.data?.length
          ? `${devApiVersion}/logo/${logoData.data[0].logo_aid}` // update
          : `${devApiVersion}/logo`, // create
        logoData?.data?.length ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
  ...
  const initVal = {
    isUpdateNavigation: itemEdit,
    logo_image: logoData?.data?.[0]?.logo_image ?? "",
    logo_name: logoData?.data?.[0]?.logo_name ?? "",
    logo_position: logoData?.data?.[0]?.logo_position ?? "",
    logo_nav_a: logoData?.data?.[0]?.logo_nav_a ?? "",
    logo_nav_b: logoData?.data?.[0]?.logo_nav_b ?? "",
    logo_nav_c: logoData?.data?.[0]?.logo_nav_c ?? "",
    logo_nav_d: logoData?.data?.[0]?.logo_nav_d ?? "",
    logo_nav_e: logoData?.data?.[0]?.logo_nav_e ?? "",
    logo_nav_f: logoData?.data?.[0]?.logo_nav_f ?? "",

    logo_image_old: logoData?.data?.[0]?.logo_image ?? "",
    pendingDeleteFile: [],
  };
    ...
    <>
      ...
      >
        <div className="modal-title">
          <h2 className="text-sm">{itemEdit ? "Edit" : "Add"} Navigation</h2>
        ...
        <div className="modal-content">
          <Formik
            ...
            onSubmit={async (values, { setSubmitting, resetForm }) => {
              const data = {
                ...values,
                logo_image: Array.from(photoArrayList).map((item) =>
                  JSON.stringify({
                    name: item.name,
                    id: item?.id || "",
                  })
              ...
              return (
                <Form>
                  <div className="modal-form">
                    <div className="modal_container mb-[50px]">
                      <div className="mt-5">
