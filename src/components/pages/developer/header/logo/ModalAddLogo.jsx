...
const ModalAddLogo = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `${devApiVersion}/logo/${itemEdit.logo_aid}` // Update
  ...
  React.useEffect(() => {
    setAnimate("");
    if (itemEdit) {
      const photos = getConvertStringToJSONparseData(itemEdit.logo_image);
  ...
  const initVal = {
    logo_image: itemEdit ? itemEdit.logo_image : "",
    logo_name: itemEdit ? itemEdit.logo_name : "",
    logo_position: itemEdit ? itemEdit.logo_position : "",

    logo_image_old: itemEdit ? itemEdit.logo_image : "",
    pendingDeleteFile: [],
  };
