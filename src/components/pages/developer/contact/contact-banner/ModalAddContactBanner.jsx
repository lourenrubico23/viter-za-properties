...
const ModalAddContactBanner = ({ itemEdit, bannerData }) => {
    ...
    mutationFn: (values) =>
      queryData(
        bannerData?.data?.length
          ? `${devApiVersion}/banner/${bannerData.data[4].banner_aid}` // update
  ...
  React.useEffect(() => {
    ...
    if (bannerData) {
      const photos = getConvertStringToJSONparseData(
        bannerData?.data?.[4]?.banner_image
      );
  ...
  const initVal = {
    ...
    banner_image: bannerData?.data?.[4]?.banner_image ?? "",
    banner_page: bannerData?.data?.[4]?.banner_page ?? "Contact",
    banner_title: bannerData?.data?.[4]?.banner_title ?? "",

    banner_image_old: bannerData?.data?.[4]?.banner_image ?? "",
    pendingDeleteFile: [],
  };
