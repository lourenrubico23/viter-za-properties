...
import ContactBannerTable from "./contact-banner/ContactBannerTable";
import ModalAddContactBanner from "./contact-banner/ModalAddContactBanner";
import ModalAddLinks from "../header/links/ModalAddLinks";
import ModalAddLogo from "../header/logo/ModalAddLogo";
import ContactContactUsTable from "./contact-contactUs/ContactContactUsTable";
import ContactNoTable from "../header/contact-no/ContactNoTable";
import ModalAddContactContactUs from "./contact-contactUs/ModalAddContactContactUs";
...
const Contact = () => {
  ...
  const handleAddBanner = () => {
    dispatch(setIsAdd({ modal: true, modalCode: "contact-banner" }));
    setItemEdit("homeBannerUpdate");
  };

  const handleAddContactUs = () => {
    dispatch(setIsAdd({ modal: true, modalCode: "contact-contactUs" }));
  ...
  return (
    <>
      <div className=" bg-[#f5f5f3] ">
        <Navigation menu="contact" submenu="" />
      ...
      {store.isAdd?.modal && store.isAdd?.modalCode === "contact-banner" && (
        <ModalAddContactBanner itemEdit={itemEdit} bannerData={bannerData} />
      )}
      {store.isAdd?.modal && store.isAdd?.modalCode === "contact-contactUs" && (
        <ModalAddContactContactUs
          itemEdit={itemEdit}
          contactFormData={contactFormData}
        />
      )}
