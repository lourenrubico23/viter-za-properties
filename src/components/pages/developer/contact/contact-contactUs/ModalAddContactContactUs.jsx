...
const ModalAddContactContactUs = ({ itemEdit, contactFormData }) => {
    ...
    mutationFn: (values) =>
      queryData(
        contactFormData?.data?.length
          ? `${devApiVersion}/contact-form/${contactFormData.data[3].form_aid}` // update
  ...
  const initVal = {
    ...
    form_page: contactFormData?.data?.[3]?.form_page ?? "Contact",
    ...
    form_contact_description:
      contactFormData?.data?.[3]?.form_contact_description ?? "",
    form_contact_disclaimer:
      contactFormData?.data?.[3]?.form_contact_disclaimer ?? "",
  };
...
export default ModalAddContactContactUs;
