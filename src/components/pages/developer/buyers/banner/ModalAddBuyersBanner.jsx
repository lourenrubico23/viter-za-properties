...
const ModalAddBuyersBanner = ({ itemEdit, bannerData }) => {
    ...
    mutationFn: (values) =>
      queryData(
        bannerData?.data?.length
          ? `${devApiVersion}/banner/${bannerData.data[2].banner_aid}` // update
  ...
  React.useEffect(() => {
    ...
    if (bannerData) {
      const photos = getConvertStringToJSONparseData(
        bannerData?.data?.[2]?.banner_image
      );
  ...
  const initVal = {
    ...
    banner_image: bannerData?.data?.[2]?.banner_image ?? "",
    banner_page: bannerData?.data?.[2]?.banner_page ?? "Buyers",
    banner_title: bannerData?.data?.[2]?.banner_title ?? "",

    banner_image_old: bannerData?.data?.[2]?.banner_image ?? "",
    pendingDeleteFile: [],
  };
                  ...
                  <div className="modal_container overflow-y-auto overflow-x-hidden h-[100dvh]">
                    ...
                    <div className="input-wrapper">
                      <InputSelect
                        ...
                        name="banner_page"
                        disabled={
                          mutation.isPending || initVal.banner_page === "Buyers"
                        } // Disable if Home
                      >
...
export default ModalAddBuyersBanner;
